<script lang="ts">
	...
	function getLargerGoogleProfileImage(url: string | undefined) {
		if (url?.includes('googleusercontent')) {
			const split_url = url?.split('=s');
			return `${split_url[0]}=s512`;
		}
		return url;
	}
...
<AuthCheck>
	...
	<form action="" class="w-full max-w-screen-md">
		<div class="form-control mx-auto my-10 w-full max-w-xs text-center">
			{#if previewURL || $userData?.photoURL}
				<img
					src={previewURL ?? getLargerGoogleProfileImage($userData?.photoURL)}
					alt="photoURL"
					width="256"
					height="256"
					class="mx-auto"
